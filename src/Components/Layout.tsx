...
function Layout(props:{search:string,setSearch:(search:string)=>void,usersSearch:string,setUsersSearch:(usersSearch:string)=>void,setLogOut:any,logOut:boolean,acceptInvatation:number,setAcceptInvatation:(acceptInvatation:number)=>void}){
    ...
    const updateLayout = useCallback(() => {
      ...
      setContentW(viewportWidth >= 1024 && !isSpecialPage ? `${viewportWidth - 167}px` : "100%");
      setHeaderHeight(
    ...
    useEffect(() => {
      ...
      // headerResize()
      updateLayout()
      window.addEventListener('resize', ()=>{      console.log('windown eventlistener')
        headerResize()} )
    }, []);
    ...
    return(
        <>
        <div className={`w-[100vw] ${(location.pathname == '/matchmake' || location.pathname == '/messenger' ) && window.innerWidth > 1024 ?'h-screen':'min-h-screen'} relative overflow-hidden md:overflow-auto bg-[#10141E] flex flex-col md:p-[25px] ${location.pathname.includes('users') || location.pathname == '/messenger' || location.pathname.includes('Pools') ?' md:pt-[24px] lg:p-[32px]':' lg:flex-row lg:gap-[39px] lg:p-[32px] lg:pr-[36px] '}`} >
            ...
            <header ref={header} style={window.innerWidth > 1024?location.pathname.includes('users') || location.pathname == '/messenger'  || location.pathname.includes('Pools')?{height:`${headerHeight}px`}:location.pathname == '/matchmake'?{height:`${headerHeight}%`}:{height:`${headerHeight}px`}:{}} className={`w-[100%] bg-[#161D2F] p-[16px] flex items-center justify-between md:rounded-[10px] md:px-[20px] z-50 ${location.pathname.includes('users') || location.pathname == '/messenger'  || location.pathname.includes('Pools') ? ' md:mb-[24px] lg:mb-[48px] ':`lg:flex-col  lg:max-w-[96px] lg:px-[28px] lg:pt-[35.4px] lg:pb-[32px] `}`}>
                <div className={` flex ${location.pathname == '/messenger' || location.pathname.includes('users') ?'flex-row':'flex-col'} items-center gap-[20px] `} >
                ...
                <FaBell style={{color:'#fab907'}} className={` w-[32px] h-[32px] hidden ${location.pathname  == '/messenger' || location.pathname.includes('users')? 'hidden' :'lg:block'}  `} onClick={()=>{fetchNoti(),setNotificationsOpen(!notificationsOpen)}}  />
                </div>
                ...
                <FaBell style={{color:'#fab907'}} className={` w-[20px] h-[20px] md:w-[32px] md:h-[32px] ${location.pathname  == '/messenger' || location.pathname.includes('users')? 'block' :'lg:hidden'}  `} onClick={()=>{fetchNoti(),setNotificationsOpen(!notificationsOpen)}} />
                ...
                <div  className={` overflow-hidden absolute top-[80px] md:top-[110px]  right-[50px] md:right-[100px] ${ location.pathname  == '/messenger' || location.pathname.includes('users')? ' lg:top-[150px] lg:right-[150px]' :' lg:top-[70px] lg:left-[150px]'} w-[260px] md:w-[340px] h-[260px] md:h-[380px] bg-[#10141E] border-[1px] border-[#243257d5] rounded-[20px] z-40 transform transition-all duration-500 ease-in-out delay-200 ${
   notificationsOpen  
      ? "opacity-100 "
      : "opacity-0 "
  }`} >
    ...
    {notifications?.map((item:Message,i:number)=>{
      ...
      return(
        <div key={item.id} className={` cursor-pointer flex items-center gap-[10px] w-[100%] h-[25%] ${i == notifications.length-1? '':'border-b-[1px] border-b-[#243257d5] ' } p-[10px] ${item.read?"":'bg-[#1d2537]'} `} onClick={item.type == "INV"?()=>{navigate(`/matchmake`)}:item.type == "MSG"?()=>{navigate(`/messenger`),localStorage.setItem('matchUpId',item.extra)}:()=>{''}} >
          ...
          <div className="flex flex-col md:gap-[4px] " >
            ...
            <p className={`${item.type == 'MSG'?"":" hidden "} text-[10px] md:text-[14px] text-[#fff]`} >{messageContent(item.body)}</p>
            ...
            <div style={location.pathname == '/messenger'?window.innerWidth < 768 ?{width:contentW,height:window.innerHeight-57}:window.innerWidth < 1024?{width:contentW,height:window.innerHeight-88}:{width:contentW}:{width:contentW}}  className={` ${location.pathname == '/matchmake' ?'h-full':''} flex flex-col `}  >
